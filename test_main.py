...
def test_add_db_item(username, private_key):
    ...
    for key, value in Items.items():
        ...
        send_buffer.append(header.format(
            key) + dictionary_to_byte_string(value))
...
if __name__ == '__main__':
    ...
    # Transer to username
    test_ownership_change(test_username_1, private_key_1,
                          owned_item, test_username_2, public_key_2)
    input(format("Press Enter to Continue", '^100s'))

    # Transfer back
    test_ownership_change(test_username_2, private_key_2,
                          owned_item, test_username_1, public_key_1)
    input(format("Press Enter to Continue", '^100s'))
